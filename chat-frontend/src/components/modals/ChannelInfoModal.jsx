...
import AddToFolderMenu from "../chat/AddToFolderMenu"
import AutoDeletePicker, { autoDeleteLabel } from "../chat/AutoDeletePicker"
...
function ChannelInfoModal({ conversation, currentUser, allUsers, onClose, onMuteToggled, onLeft, onUpdated }) {
  ...
  const filteredNewUsers = (allUsers || [])
    ...
    .filter((u) => `${u.full_name || ""} ${u.username || ""}`.toLowerCase().includes(memberSearch.toLowerCase()))
    ...
    try {
      for (const userId of selectedNewIds) {
        try {
          ...
          failures.push(err.response?.data?.error || "Failed to add a member")
        }
      }
      fetchMembers()
      const addedCount = selectedNewIds.length - failures.length
      onUpdated?.({ ...conversation, members_count: subscriberCount + addedCount })
      setSelectedNewIds([])
      setMemberSearch("")
      setMode("view")
      if (addedCount > 0) toast.success("Members added")
          ...
          <>
            ...
            {isAdmin ? (
                ...
                {inviteLink ? (
                  <div className="channel-info-link-section">
                    ...
                    {conversation.description && (
                      <div className="channel-info-description-block">
                        <p className="channel-info-description">{conversation.description}</p>
                        <span className="channel-info-link-hint">Description</span>
                      </div>
                    )}
                    <button
              ...
              <>
                ...
                {(inviteLink || conversation.description) && (
                  <div className="channel-info-link-section">
                    {inviteLink && (
                      <div className="channel-info-link-row" onClick={copyLink} title="Copy link">
                        <span className="channel-info-link">{inviteLink}</span>
