...
import WallpaperPicker from "../modals/WallpaperPicker"
...
const buildMemberChatTarget = (member) => {
  ...
  if (existing) {
    return {
      ...
      type: "private",
      displayName: existing.other_participant.full_name || "Unknown",
  ...
  if (conversationRef.current) {
    updateConversation(conversationRef.current.id, {
      last_message: {
        ...
        sender_display_name: normalizedMsg.sender_display_name,
  ...
  if (data.success) {
    ...
    updateConversation(data.target_conversation_id, {
      last_message: {
        ...
        sender_display_name: data.message.sender_display_name,
...
setLoading(true)
  ...
  const sendMessage = (messageText, attachments = [], reply, messageType = "text") => {
    ...
    setMessages(prev => [...prev, {
      ...
      sender_display_name: currentUser.full_name || "Unknown",
      content: messageText.trim(),
              ...
              style={{ cursor: 'pointer' }}>
                ...
                <div className="result-avatar">
                  <div className="result-avatar-placeholder" style={{ background: getAvatarColor(msg.sender_username || msg.sender) }}>
                    {(msg.sender_display_name || "Unknown")[0].toUpperCase()}
                  </div>
                </div>
                <div className="result-content">
                  <div className="result-header">
                    <span className="result-sender">{msg.sender_display_name || "Unknown"}</span>
